...
def get_ubicaciones():
    with engine.connect() as conn:
        rows = conn.execute(
            text("SELECT id, nombre, piso, descripcion, qr_code FROM ubicaciones ORDER BY piso, nombre")
        ).fetchall()
    return [
...
def get_ubicacion(id: int):
    with engine.connect() as conn:
        row = conn.execute(
            text("SELECT id, nombre, piso, descripcion, qr_code FROM ubicaciones WHERE id = :id"),
            {"id": id},
        ).fetchone()
    if not row:
