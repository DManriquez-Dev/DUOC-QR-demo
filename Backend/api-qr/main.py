...
def get_qr(id: str, request: Request):
    ...
    try:
        with engine.connect() as conn:
            row = conn.execute(
                text("""
                    SELECT qr_id, qr_activo, ubicacion_id, ubicacion_nombre, piso_numero
            ...
            if not row:
                raise HTTPException(status_code=404, detail=f"QR '{codigo}' no encontrado")

            if not row.qr_activo:
                raise HTTPException(status_code=403, detail=f"QR '{codigo}' está desactivado")

            dispositivo = request.headers.get("User-Agent", None)
            conn.execute(
                text("INSERT INTO escaneos (qr_id, dispositivo) VALUES (:qr_id, :dispositivo)"),
                {"qr_id": row.qr_id, "dispositivo": dispositivo}
            )
            conn.commit()

            return {"id": row.ubicacion_id, "ubicacion": row.ubicacion_nombre, "piso": row.piso_numero}
...
def listar_usuarios(_: dict = Depends(require_admin)):
    ...
    return [{"id": r.id, "username": r.username, "email": r.email, "rol": r.rol} for r in rows]
...
def crear_usuario(req: RegisterRequest, _: dict = Depends(require_admin)):
    ...
    with engine.begin() as conn:
        existe = conn.execute(
            text("SELECT 1 FROM usuarios WHERE username = :u"), {"u": req.username}
        ).fetchone()
        if existe:
